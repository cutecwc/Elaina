...
在此处稍微介绍一下Arch系列的另一个受欢迎的分支，archlinux的arch-install脚本使用起来不太方便，属于一种食之无味弃之可惜的尴尬位置。如果不希望繁琐的使用命令行安装archlinux又不希望使用manjaro这种过分添加了额外东西的系统，不妨使用图形化安装界面，它会让你选择那些包要安装，这种安装是更加可控的。
...
-肉眼可见的包数量变多了（相比于Archlinux），上面的软件包配置完成后，Manjaro（约1100）>EndeavourOS（约950）>Archlinux（850）。还可以接受，就懒得清理了0.0. 。
